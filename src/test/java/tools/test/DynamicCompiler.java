...
public class DynamicCompiler {
	...
	private static String classOutputFolder = "./classes";
	...
	public static class InMemoryJavaFileObject extends SimpleJavaFileObject {
		...
		/**
		 * className必须和类名保持一致 2014-11-23 by 六三
		 * 
		 * @param className
		 * @param contents
		 * @throws Exception
	...
	private static JavaFileObject getJavaFileObject() {
		StringBuilder contents = new StringBuilder("package math;" + "public class Calculator { "
				+ "  public void testAdd() { "
				+ "    System.out.println(200+300); System.out.println(tools.Convert.toString(2)); " + "  } "
				+ "  public static void main(String[] args) { " + "    Calculator cal = new Calculator(); "
				+ "    cal.testAdd(); " + "  } " + "} ");
		JavaFileObject so = null;
		try {
			so = new InMemoryJavaFileObject("Calculator", contents.toString());
	...
	public static void compile(Iterable<? extends JavaFileObject> files) {
		...
		List<String> options = Arrays.asList("-d", classOutputFolder);
	 ...
	 * @throws Throwable
	 */
	public static void runIt() throws Throwable {
